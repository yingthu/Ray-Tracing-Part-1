...
/**
 * Each pixel requires a few data elements to compute the color.  Allocating them for
 * each new pixel can be quite costly.  Further, each pixel is completely independent,
 * so the data space could be reused for each pixel without causing any computation errors.
